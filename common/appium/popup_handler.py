...
MAX_CLOSE_PER_SCAN = 2      # 单次扫描最多关闭的弹窗数（防嵌套弹窗）
...
class PopupHandler:
    ...
    def __init__(self, driver, logger=None):
        ...
        self._driver = driver
        self._log = logger
        self.enabled = os.environ.get('POPUP_AUTO_CLOSE', '1') != '0'
    ...
    def scan(self):
        ...
        if not getattr(self, 'enabled', False) or not self._rules:
            return 0
        now = time.time()
        if now - getattr(self, '_last_scan', 0) < MIN_SCAN_INTERVAL:
            return 0
        self._last_scan = now
        closed = 0
        activity = None
        for rule in self._rules:
            if closed >= MAX_CLOSE_PER_SCAN:
                break
